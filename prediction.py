...
def predict_from_text(new_text, folder_save):
    ...
    #Print the results
    print(f'According to Logreg, this poetry was song by {clf_logistic.predict(X_test)[0].upper()}')

    print("The probabilities are : \n")
    print(f'{clf_logistic.classes_} \n')
    print(f'{clf_logistic.predict_proba(X_test)} \n')

    print(f'According to Bayes, this poetry was song by {clf_bayes.predict(X_test)[0].upper()}')
